            ...
            for row in csv_reader:
                ...
                if counter == 1:
                    for index, value in enumerate(row):
                        if (
                            (capture_belongs_str in value)
                            and (capture_chn_str in value)
                            and (capture_stop_number_str not in value)
                            and value != ""
                        ):
...
def code_data(
    data_list, addr_dic, current_group_keywords, group_keywords, addr_type_list
):
    output = []
    counter = 0
    ...
    for line in data_list:
        ...
        if addr_dy in addr_dic:
            for addresses_item in sorted(
                ...
                if (
                    addr_name in addresses_item_name_chn
                    or addresses_item_name_chn in addr_name
                ) and (matched_time == 1):
                    matched_term = match_belongs_info(
                        addresses_item_belong_list, addr_belong
                    )
                    if addr_name == addresses_item_name_chn and matched_term != "":
                        exact_with_belong_match_list.append(
                            [
                                addresses_addr_id,
                                "exact_with_belong",
                                addresses_item_name_chn,
                                matched_term,
                            ]
                        )
                    elif (
                        addr_name != addresses_item_name_chn
                        and matched_term != ""
                        and (
                            addr_name.find(addresses_item_name_chn) == 1
                            or addresses_item_name_chn.find(addr_name) == 1
                        )
                    ):
                        partial_with_belong_match_list.append(
                            [
                                addresses_addr_id,
                                "partial_with_belong_dangerous",
                                addresses_item_name_chn,
                                matched_term,
                            ]
                        )
                        ...
                        partial_without_belong_match_list.append(
                            [
                                addresses_addr_id,
                                "partial_without_belong_dangerous",
                                addresses_item_name_chn,
                                matched_term,
                            ]
                        )
                ...
                # remove the type the address name and try to match again
                    ...
                    for addr_type in addr_type_list:
                        ...
                        if len(addresses_item_name_chn_short) <= 1:
                            continue
                        if (
                            addr_name_short in addresses_item_name_chn_short
                            or addresses_item_name_chn_short in addr_name_short
                        ) and (matched_time == 1):
                            matched_term = match_belongs_info(
                                addresses_item_belong_list, addr_belong
                            )
                            if matched_term != "" and (
                                addr_name_short.find(addresses_item_name_chn_short) == 1
                                or addresses_item_name_chn_short.find(addr_name_short)
                                == 1
                            ):
                                partial_with_belong_match_list.append(
                                    [
